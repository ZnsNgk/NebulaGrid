...
对于需要本地编译或 CUDA 编译的包，用户可选择“在指定节点”执行安装。用户每次打开选择弹窗时，系统都应实时探测主节点和用户可见计算节点的 `gcc`、`g++`、`clang`、`nvcc` 版本以及 GPU 清单；未安装的编译器在页面中显示“未安装”。用户可选择“默认”“CPU”或多选具体 GPU：默认模式不设置 `CUDA_VISIBLE_DEVICES`，CPU 模式设置为目标节点 GPU 总数加一，指定 GPU 模式设置为用户选择的 GPU index 列表。编译安装属于环境维护作业，不进入普通任务等待队列，不由任务调度器分配资源，也不计入任务资源占用；但系统必须记录编译作业、执行节点、可见 GPU、远端 PID、日志和审计事件，管理员可查看或中止。

为避免环境污染，每次包安装应写入 env_install_jobs 与 env_package_manifests：记录上传包、目标环境、安装命令、工作目录、编译目标、GPU 可见模式、安装前后 pip freeze 差异、导入文件清单、返回码和日志路径。安装失败时不得把环境标记为 available，前端应显示失败原因并允许用户下载日志。同一环境同一时间只允许一个安装包或删除包操作；环境忙碌时安装包页面状态显示为“安装中”，继续提交应提示“该环境正在进行操作，请稍后”。
