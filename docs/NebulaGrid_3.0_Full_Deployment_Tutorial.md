...
- `sharing_scope=none` 表示不共享，仅所有人和管理员可见；`group` 表示组内共享：学生所有人的导师及这些导师名下所有学生均可见可用，导师所有人的节点则共享给其名下学生；`public` 表示所有用户可见可用。所有人始终保留访问权，多所有人或多导师按各自共享范围取并集，组外用户不因组内共享获得访问权。
- 共享范围同时约束普通用户的节点可见性、任务指定节点校验和自动调度候选范围；实际分配还需满足节点在线、开启调度及资源可用等条件。不可调度 GPU 不计入 GPU 总数、可用 GPU 数，也不会作为任务 GPU 型号选项的来源。
