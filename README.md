...
新增侧边栏 `用量统计`，提供任务统计、节点统计和按角色授权的用户用量明细：

- **任务统计**：查看提交、成功、失败、取消等数量，以及总运行时长和 GPU 执行卡时；导师可查看本人及名下学生，管理员可查看所有用户。
- **节点统计**：支持近 7/30/90 天，展示当前可见节点的 CPU/GPU 平均使用率、平台占用率、日均使用时长和个人占用时长。
- **小时汇总**：在独立 InfluxDB bucket `nebulagrid_usage_hourly` 保存每小时 CPU/GPU 均值，保留 90 天；启动后后台补算缺失历史，每小时续算。页面仅读取汇总，不计当前未结束小时。
- **缺失数据处理**：某天只有 `23 / 24` 小时汇总时，按已有 23 小时计算日均及周期加权，保留完整度提示；已保存的零值继续参与平均。兼容首次查找新 bucket 返回 404 的初始化情况。
- **用户占用卡时**：导师及管理员的本期用户汇总新增 GPU 占用卡时；点击服务器后，可查看该节点各授权用户的占用时长和卡时，仅列出本周期占用时长大于 0 的用户，无记录时列表为空。

节点页卡时按实际 GPU 分配到释放计算，同一用户在同一节点复用同一卡的重叠时间去重；任务页卡时仅计程序执行时间。统计基于仍保留的任务、分配和监控记录，已删除或过期的历史无法恢复。

升级需配套更新前后端并重启 `nebulagrid-api`，小时汇总进程由 API 自动托管，无须新增 systemd 服务。InfluxDB token 须允许创建汇总 bucket，或由管理员预建 90 天保留期的 bucket 并授予读写权限；首次补算在后台进行。若环境文件显式设置了 `NEBULAGRID_APP_VERSION`，请同步改为 `1.0.4`。详细步骤见[完整部署教程 §17.11](./docs/NebulaGrid_3.0_Full_Deployment_Tutorial.md#1711-用量统计部署与验收)。

## 历史版本记录

> 以下日期按仓库相关提交和开发日志整理，用于标记版本里程碑；仓库尚未建立对应的版本标签。

| 版本 | 日期 | 主要更新 |
| --- | --- | --- |
| **v1.0.4（当前）** | 2026-09-15 | 新增任务与节点用量统计、90 天小时监控汇总及自动补算、用户 GPU 占用卡时与节点用户明细；修复 bucket 首次初始化 404 和日内小时缺失导致整天无数据的问题。 |
| v1.0.3 | 2026-09-07 | 新增任务进度识别与剩余时间预估、GPU 剩余占用时间提示，以及历史任务运行时长展示。 |
| v1.0.2 | 2026-09-04 | 修复停止任务后无法确定远端进程是否退出的问题：增加“停止中”确认流程，确认退出后显示“已停止”，确认超时则标记“状态未知”；完善远端进程回收与任务状态同步。 |
| v1.0.1 | 2026-05-27 | 优化前端界面与操作体验，修复文件管理、任务执行和调度等功能中的 Bug。 |
| v1.0.0 | 2026-05-24 | 试运行版本：完成首轮实机部署与核心链路联调，提供节点、用户、文件、环境和任务管理等基础功能。 |

---

本文档根据 `docs/` 目录中的需求规格说明书与系统架构设计书整理，用于指导 NebulaGrid 3.0 在实验室主控节点和计算节点上的部署、配置、启动、验证与日常运维。

完整部署文档请查看：[NebulaGrid 3.0 完整部署文档](./docs/NebulaGrid_3.0_Full_Deployment_Tutorial.md)

> 当前仓库提供 FastAPI、前端控制台、节点与用户管理、文件与环境管理、任务调度执行、进度预估及用量统计。新部署请以完整部署教程为准，完成 PostgreSQL/InfluxDB/Redis、NFS、SSH key、conda 路径和 systemd 服务联调。
