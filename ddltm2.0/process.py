...
def process(info):
    ...
    try:
        ...
        if mode == "env_list":
            ...
            send_info = json.dumps(send_info)
            return (send_info, True)
        elif mode == "get_gpu_type":
            gpu_type_list = []
            if msg:
                for slaver in conf.slaver_info:
                    if slaver["name"] == msg:
                        for gpu in slaver["gpu_info"]:
                            if not gpu in gpu_type_list:
                                gpu_type_list.append(gpu)
                        break
            else:
                for slaver in conf.slaver_info:
                    for gpu in slaver["gpu_info"]:
                        if not gpu in gpu_type_list:
                            gpu_type_list.append(gpu)
            send_info = {"mode": "ret_get_gpu_type", "ui": ui , "info": gpu_type_list}
            send_info = json.dumps(send_info)
            return (send_info, True)
        else:
            return ("", False)
