...
class Main:
    ...
    def run(self):
        try:
            ...
            # Web控制台线程：与master进程共用任务队列和节点状态，不影响原PyQt客户端。
            try:
                from web_app import run_web_app
                t6 = threading.Thread(target=run_web_app, args=(self,), daemon=True)
            except Exception as e:
                t6 = None
                log("Web控制台加载失败: "+str(e), self.config.config["server_log_path"])
