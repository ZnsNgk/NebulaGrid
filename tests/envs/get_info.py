...
def main():
    ...
    while True:
        try:
            res = {}
            res['cpu'] = [usage_to_int(value) for value in psutil.cpu_percent(percpu=True)]
            res['cpu_total'] = usage_to_int(psutil.cpu_percent())
            res['ram'] = psutil.virtual_memory()._asdict()
            res['ram']['percent'] = usage_to_int(res['ram']['percent'])
            res['ram']['available'] = bytes_to_gb(res['ram']['available'])
            ...
            try: 
                res['gpu'] = [
                    {
                        'id': gpu.id,
                        'load': usage_to_int(gpu.load * 100),
                        'mem_used': gpu.memoryUsed,
                        'mem_total': gpu.memoryTotal,
                        'mem_util': usage_to_int(gpu.memoryUtil * 100)
                    }
            ...
            if enable_nvidia:
                ...
                for device_id in range(deviceCount):
                    ...
                    for ps in cps :
                        ...
                        gpu_proc.append({'name': name, 'pid': pp, 'mem_gpu': ps.usedGpuMemory, 'cpu': usage_to_int(proc.cpu_percent()), 'ram': proc.memory_info().vms, 'username': proc.username()})
